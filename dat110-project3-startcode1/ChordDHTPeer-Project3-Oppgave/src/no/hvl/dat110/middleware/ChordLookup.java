...
public class ChordLookup {
	...
	public NodeInterface findSuccessor(BigInteger key) throws RemoteException {
		...
		BigInteger nodeIdPlusOne = node.getNodeID().add(BigInteger.ONE);
		if(Util.computeLogic(key, nodeIdPlusOne, succStub.getNodeID())) {
	...
	private NodeInterface findHighestPredecessor(BigInteger key) throws RemoteException {
		...
		for(int i = fingerTable.size() - 1; i >= 0; i--) {
			...
			NodeInterface fingerStub = Util.getProcessStub(nodeI.getNodeName(), nodeI.getPort());
			
			BigInteger nodeIdPlusOne = node.getNodeID().add(BigInteger.valueOf(1));
			
			BigInteger keyMinusOne = key.subtract(BigInteger.valueOf(1));

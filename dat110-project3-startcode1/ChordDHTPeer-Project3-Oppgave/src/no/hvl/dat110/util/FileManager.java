...
public class FileManager {
	...
	public void createReplicaFiles() {
		...
		for(int i = 0; i < numReplicas; i++)
		{
			...
			// hash the replica
			BigInteger replicafilenameHashed = Hash.hashOf(replicafilename);
			
			// store the hash in the replicafiles array.
			replicafiles[i] = replicafilenameHashed;
		}
    ...
    public int distributeReplicastoPeers() throws RemoteException {
    	...
    	for (BigInteger file : replicafiles) {
    		...
    		if(counter == index) 
    			succ.saveFileContent(filename, file, bytesOfFile, true);
    		else 
    			succ.saveFileContent(filename, file, bytesOfFile, false);
	...
	public Set<Message> requestActiveNodesForFile(String filename) throws RemoteException {
		...
		for (BigInteger file : replicafiles) {
    		...
    		Message meta = succ.getFilesMetadata(file);
